...
<body id="devisToPdf">
    <div class="header container">
        ...
        <div class="container-date">
            <p class="fs-5 text text-date"> Le {{date('d/m/Y')}} à Casablanca</p>
        </div>
    </div>
    <div class="container">
        <div class="cards">
            <div class="card mt-2 d-flex flex-row">
                    ...
                    <p class="cardValue">{{strtoupper($nomEntreprise)}}</p>
            </div>
            <div class="card mt-2 d-flex flex-row m-auto" >
                <p for="">DEVIS NUMERO :</p>
                <p class="cardValue"> {{$numeroDevisFormater}}</p>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="cards">
            <div class="card mt-2 d-flex flex-row">
                    <p for="">Adresse :</p>
                    <p class="cardValue">{{$adresse}}
                    </p>
            </div>
            <div class="card mt-2 d-flex flex-row m-auto" >
                <p for="">Nom/Prénom du contact :</p>
                <p class="cardValue">{{$nomcontact}}</p>
            </div>
        </div>
    </div>
    <div class="container tableau">
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($devis as $k => $devi)
                    <tr>
                    <th scope="row">{{$devi['designation']}}</td>
                    <td>{{$devi['nb_sac_par_carton']}}</td>
                    <td>{{$devi['nb_tot_carton']}}</td>
                    <td>{{$devi['nb_tot_sac']}}</td>
                    <td>{{number_format($devi['ptr_prix_par_sac_dap'],2)}}</td>
                    <td>{{$devi["nb_sac_par_carton"] * number_format($devi["ptr_prix_par_sac_dap"], 2)}}</td>
                    <td>{{$devi["nb_tot_sac"] * number_format($devi["ptr_prix_par_sac_dap"], 2)}}</td>
                </tr>
                @endforeach
            </tbody>
          </table>
